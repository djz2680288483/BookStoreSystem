        ...
        <div class="col-md-12 column">
            <div class="page-header">
                <h1 style="text-align: center">
                    <small style="font-weight: bold">我的订单</small>
                </h1>
            </div>
        </div>
    </div>
                            ...
                            <c:choose>
                                <c:when test="${order.isPay==0}">
                                    <form action="${pageContext.request.contextPath}/order/Pay" method="post">
                                        ...
                                        <button type="submit" class="btn button btn-warning">付款</button>
                                    </form>
                                </c:when>
                                <c:when test="${order.isPay==1&&order.isDelivery==0}"><span style="color: #999999;font-weight: bold">待发货</span></c:when>
                                <c:when test="${order.isDelivery==1&&order.completion==0}">
                                    <form action="${pageContext.request.contextPath}/order/Completion" method="post">
                                        <input type="hidden" name="id" value="${order.id}">
                                        <button type="submit" class="btn button btn-warning">点击收货</button>
                                    </form>
                                </c:when>
                                <c:when test="${order.completion==1}"><button class="btn btn-success">订单完成</button></c:when>
                            </c:choose>
                        </td>
                    </tr>
                </c:forEach>
